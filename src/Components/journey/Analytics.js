...
function Analytics(props) {
  ...
  const calculateTotalTime = ({ tracks }) => {
    ...
    const percentageDistracted = (
      (totalDistracted / (totalDistracted + totalFocused)) *
      100
    ).toFixed(2);
    const percentageFocused = (
      (totalFocused / (totalDistracted + totalFocused)) *
      100
    ).toFixed(2);
  ...
  const trackTime = ({ tracks }) => {
    let distractions = [];
    let focused = [];
    let labels = [];

    const durationFormat = findLongestTime(tracks);

    tracks.map(track => {
      labels = [...labels, track.skill.title];
      distractions = [
        ...distractions,
        track.distraction * durationFormat.convertor
      ];
  ...
  const durationIdentifier = highestTime => {
    ...
    const durationFormat = highestTime
  ...
  return (
    <>
      {props.tracks ? (
        <div
          class="uk-grid-small uk-grid-divider uk-child-width-1-2@m"
          data-uk-grid
        >
